...
export default function CommandCenter() {
  ...
  return <div className={`page-stack ${isAdministrator ? "" : "page-stack--officer"}`}>
    <section className="hero-panel"><div className="hero-panel__content"><div className={`eyebrow ${isAdministrator ? "eyebrow--light" : ""}`}><Sparkles size={14} /> {isAdministrator ? "Safeguarding oversight" : "PRIVATE CASEWORK"}</div><h1>{isAdministrator ? <>A clear view of<br /><em>responsible safeguarding.</em></> : <>Careful follow-up.<br /><em>Clear next actions.</em></>}</h1><p>{isAdministrator ? "Track reporting intake, priority signals, and accountable oversight across the live safeguarding service." : "Your officer workspace keeps each available case, record update, and safe handoff in one protected place."}</p><div className="hero-panel__actions"><button className="button button--primary" onClick={() => navigate(user?.role === "officer" ? "/cases" : "/reports")}>{user?.role === "officer" ? "Open my case queue" : "Review reporting intake"} <ArrowRight size={17} /></button><button className={`button ${isAdministrator ? "button--ghost-light" : "button--outline"}`} onClick={() => casesQuery.refetch()}>{casesQuery.isFetching ? "Refreshing…" : "Refresh live data"}</button></div></div><div className="hero-panel__signal"><span className="signal-dot" /><div><small>{isAdministrator ? "Protected workspace" : "Your secure space"}</small><strong>{casesQuery.isSuccess ? "Live Django records available" : "Checking safeguarding records"}</strong></div></div></section>
