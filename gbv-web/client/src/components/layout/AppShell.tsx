...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell">
    <aside className={`command-rail ${menuOpen ? "command-rail--open" : ""}`}>
    ...
    <main className="workbench"><header className="topbar"><button className="menu-toggle" aria-label="Open navigation" onClick={() => setMenuOpen(true)}><Menu size={21} /></button><button className="search-trigger" onClick={() => toast.info("Use the live case queue search to find records available to your safeguarding role.")}><Search size={18} /><span>Search live casework</span><kbd>⌘ K</kbd></button><div className="topbar-actions"><button className="icon-button" aria-label="Notifications" onClick={() => toast.info("Notification delivery remains managed by the Django safeguarding service.")}><Bell size={19} /><span className="notification-dot" /></button><button className="new-action" onClick={() => navigate(primaryAction.href)}><Plus size={18} /> {primaryAction.label}</button></div></header><div className="workspace-banner"><span className="workspace-banner__signal" /> LIVE DJANGO WORKSPACE <span>Connected to the local safeguarding API. Handle all records according to approved privacy policy.</span></div><section className="page-frame">{children}</section></main>
  </div>;
}
